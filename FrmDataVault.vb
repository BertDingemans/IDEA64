...
Public Class FrmDataVault
    ...
    Private Sub ButtonAdd2Diagram_Click(sender As Object, e As EventArgs) Handles ButtonAdd2Diagram.Click
        ...
        Try
            ...
            Me.Repository.Execute(String.Format("UPDATE t_object SET package_id = {1}, name = '{2}', style='{3}' WHERE object_id = {0} ",
                                                objElement.ElementID, objDiagram.PackageID,
                                                objElement.Name.Replace(Me.TextBoxTemplateReplaceValue.Text, Me.TextBoxName.Text).Replace("--", "").Replace("__", ""),
                                                objElement.StyleEx.Replace("Locked=true;", "")))
            For Each Attribute As EA.Attribute In objElement.Attributes
                Attribute.Name = Attribute.Name.Replace(Me.TextBoxTemplateReplaceValue.Text, Me.TextBoxName.Text)
                Attribute.Name = Attribute.Name.Replace("--", "")
                Attribute.Name = Attribute.Name.Replace("__", "")
                Attribute.Update()
            Next
            For Each Operation As EA.Method In objElement.Methods
                Operation.Name = Operation.Name.Replace(Me.TextBoxTemplateReplaceValue.Text, Me.TextBoxName.Text)
                Operation.Name = Operation.Name.Replace("--", "")
                Operation.Name = Operation.Name.Replace("__", "")
                Operation.Update()
                For Each Param As EA.Parameter In Operation.Parameters
                    Param.Name = Param.Name.Replace(Me.TextBoxTemplateReplaceValue.Text, Me.TextBoxName.Text).Replace("--", "").Replace("__", "")
                    Param.Update()
                Next
            Next
