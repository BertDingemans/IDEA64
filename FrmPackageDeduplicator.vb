...
Public Class FrmPackageDeduplicator
    ...
    Private oPackage As EA.Package = Me.Repository.GetTreeSelectedPackage()
    Private sPackageList As String = ""
    Private oDef As New IDEADefinitions()
    Private blnDiagram As Boolean = False
    Public Property isDiagram() As Boolean
        Get
            Return blnDiagram
        End Get
    ...
    Sub LoadGrid()
        ...
        Try
            ...
            'load elements grid
            If blnDiagram Then
                strSql = oDef.GetSettingValue("DeduplicateElementsDiagram")
                Dim objDiagram As EA.Diagram = Me.Repository.GetCurrentDiagram()
                If Not IsNothing(objDiagram) Then
                    DataGridViewElements.DataSource = DLA2EAHelper.SQL2DataTable(strSql.Replace("#diagram_id#", objDiagram.DiagramID.ToString()), Me.Repository)
                    Me.ResultSplitContainer.Panel2Collapsed = True
                    DataGridViewConnectors.DataSource = New DataTable()
                    Me.Text = "Deduplicate diagram " + objDiagram.Name
                End If
            Else
            ...
            End If
            strSql = oDef.GetSettingValue("DeduplicateElementsPackage")
            If DataGridViewElements.Columns.Count > 0 Then
                DataGridViewElements.Columns("origid").Visible = False
                DataGridViewElements.Columns("duplid").Visible = False
            End If
            If DataGridViewConnectors.Columns.Count > 0 Then
                DataGridViewConnectors.Columns("origid").Visible = False
                DataGridViewConnectors.Columns("duplid").Visible = False
            End If
    ...
    Sub DeDuplicate()
        ...
        Try
            ...
            If Me.CheckBoxBaseLine.Checked Then
                Me.Repository.GetProjectInterface().CreateBaseline(oPackage.PackageGUID, "Baseline", "Baseline before deduplicator")
                Me.AddMessage("Baseline created")
            End If
    ...
    Private Sub FrmPackageDeduplicator_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        ...
        DataGridViewElements.DefaultCellStyle.WrapMode = DataGridViewTriState.False
        DataGridViewElements.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.DisplayedCells
        DataGridViewElements.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells

        DataGridViewConnectors.DefaultCellStyle.WrapMode = DataGridViewTriState.False
        DataGridViewConnectors.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.DisplayedCells
        DataGridViewConnectors.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        Me.LoadGrid()
        Me.Savetranslations()
    ...
    Private Sub ButtonSaveValidation_Click_1(sender As Object, e As EventArgs) Handles ButtonSaveValidation.Click
        ...
        My.Settings.Save()
    End Sub
    Private Sub ToolStripButtonDeduplicate_Click(sender As Object, e As EventArgs) Handles ToolStripButtonDeduplicate.Click
        Dim oDef As New IDEADefinitions()
        If Me.Repository.IsSecurityEnabled = False Or DLA2EAHelper.IsUserGroupMember(Me.Repository, oDef.GetSettingValue("AuthorizationGroup")) Then
    ...
    Private Sub ToolStripButtonOriginal_Click(sender As Object, e As EventArgs) Handles ToolStripButtonOriginal.Click
    ...
    Private Sub ToolStripButtonDuplicate_Click(sender As Object, e As EventArgs) Handles ToolStripButtonDuplicate.Click
        Dim oDVRTX As System.Windows.Forms.DataGridViewTextBoxCell
        Try
    ...
    Private Sub ToolStripButtonDuplicateReport_Click(sender As Object, e As EventArgs) Handles ToolStripButtonDuplicateReport.Click
        Try
