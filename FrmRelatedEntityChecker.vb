...
Public Class FrmRelatedEntityChecker
    ...
    Protected strElementSQL As String =
        "SELECT 'Element'as EntityType
    , Original.name as original_name
    , Original.status as status_original
    , Related.name as related_element
    , Related.status as status_related
    , t_package.name as related_package
    FROM t_object as Original 
    ...
    Function SetElement(oElement As EA.Element) As Boolean
        ...
        Return False
    End Function
    Private Sub ProcessColorGrid(strColumn As String, oRow As System.Windows.Forms.DataGridViewRow)
        Dim strStatus As String

        If Not IsNothing(oRow.Cells(strColumn).Value) Then
            strStatus = oRow.Cells(strColumn).Value.ToString().ToUpper()
            Select Case strStatus
                Case "REFERENCE", "IMPLEMENTED", "VALIDATED"
                    oRow.Cells(strColumn).Style.BackColor = Color.IndianRed
                    Me.CheckBoxContinue.Checked = False
                    Me.CheckBoxContinue.BackColor = Color.Magenta
                    Me.CheckBoxContinue.Text = Me.CheckBoxContinue.Text.ToUpper()
                Case "SOLUTION", "APPROVED"
                    oRow.Cells(strColumn).Style.BackColor = Color.LightSalmon
                Case "PROPOSED"
                    oRow.Cells(strColumn).Style.BackColor = Color.LightGreen
                Case Else
    ...
    Private Sub ColorGrid(ByVal Grid As DataGridView)
        ...
        Try
            For Each oRow In Grid.Rows
                Me.ProcessColorGrid("status_original", oRow)
                Me.ProcessColorGrid("status_related", oRow)
            Next
        Catch ex As Exception
            DLA2EAHelper.Error2Log(ex)
        End Try
