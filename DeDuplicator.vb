...
Namespace DLAFormfactory
    ...
    Public Class DeDuplicator
        ...
        End Function
        ''' <summary>
        ''' Connection to the open and active repostory
        ''' </summary>
        ''' <returns></returns>
        ...
        Public Sub DeduplicateElement(ByVal strOriginalID As String, ByVal strDuplicateID As String, ByVal strPackageID As String, ByVal strDuplPackageId As String)
            ...
            Try
                ...
                Else
                    ...
                    strDiagramSQL = {"DELETE FROM t_diagramobjects WHERE object_id = #dupl_id# AND EXISTS(SELECT 1 FROM t_diagramobjects AS DO 
                        WHERE DO.object_id = #orig_id# AND DO.diagram_id = t_diagramobjects.diagram_id)  ",
                       "UPDATE t_diagramobjects SET object_id = #orig_id# 
                        WHERE object_id = #dupl_id# AND NOT EXISTS(SELECT 1 FROM t_diagramobjects AS DO 
                        WHERE DO.object_id = #orig_id# AND DO.diagram_id = t_diagramobjects.diagram_id)  "}
                    ...
                    If Me.HasModule("TaggedValues") Then
                        For Each objTaggedValue In objElement.TaggedValues
                            Dim strNaam As String = objTaggedValue.Name
                            Dim blnFound As Boolean = False
                            For Each objLinksTaggedValue As TaggedValue In objElementLeft.TaggedValues
                                If objLinksTaggedValue.Name = strNaam Then
                                    blnFound = True
                                    Exit For
                                End If
                            Next
