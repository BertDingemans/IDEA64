...
Namespace DLAFormfactory
    ...
    Public Class DLADataSetContainer
        ...
        ''' </summary>
        ''' <param name="Name">Name of the table in the dataset</param>
        ''' <param name="DT"></param>
        ''' <returns></returns>
        Public Function AddTable(Name As String, DT As DataTable) As DataTable
            Try
                DT.TableName = Name
                Me.objDS.Tables.Add(DT.Copy())
                Return DT
        ...
        End Function
        ''' <summary>
        ''' Add extra tables to the dataet for various system settings of the simulator
        ''' </summary>
        ''' <returns></returns>
        Public Function AddExtraTable() As Boolean
            Dim DT As DataTable
            Try
                DT = DLA2EAHelper.SQL2DataTable("select target.name as actor_name
                                                , target.object_type as actor_type
                                                , source.name as class_name
                                                , source.object_type as class_type
                                                FROM t_object as source
                                                , t_object as target
                                                , t_connector
                                                WHERE source.object_id = t_connector.start_object_id
                                                AND target.object_id = t_connector.end_object_id
                                                AND target.object_type  = 'Actor'
                                                AND source.object_type = 'Class'
                                                ...
                                                WHERE source.object_id = t_connector.start_object_id
                                                AND target.object_id = t_connector.end_object_id
                                                AND target.object_type  = 'Class'
                                                AND source.object_type = 'Actor'
                                                AND source.object_type <> target.object_type
                                                ", Me.Repository)
                Me.AddTable("SYS_AUTHORIZATION", DT)

                DT = DLA2EAHelper.SQL2DataTable("select source.name as source_name
                                                , source.object_type as source_type
                                                , target.name as target_name
                                                , target.object_type as target_type
                                                FROM t_object as source
                                                ...
                                                WHERE source.object_id = t_connector.start_object_id
                                                AND target.object_id = t_connector.end_object_id
                                                AND source.object_type IN('Actor', 'Action', 'Decision', 'StateNode')
                                                AND target.object_type IN('Actor', 'Action', 'Decision', 'StateNode','Interface', 'Class') ", Me.Repository)
                Me.AddTable("SYS_PROCESS", DT)
                Return True
            Catch ex As Exception
                DLA2EAHelper.Error2Log(ex)
                Return False
        ...
        End Function
        ''' <summary>
        ''' Add a relation between the parent and the child object and combine it with a defined lookup
        ''' </summary>
        ''' <param name="parent"></param>
        ''' <param name="Child"></param>
        ''' <param name="lookup"></param>
        ''' <returns></returns>
        Public Function AddRelation(parent As String, Child As String, lookup As String) As Boolean
            Dim oRow As DataRow
            Try
                Dim oDT As DataTable
                oDT = Me.GetCommandForTable(Child, "LOOKUP")
                If oDT.Rows.Count = 1 Then
                    Dim LookupRow As DataRow = oDT.Rows(0)
                    lookup = LookupRow("statement")
                    'dit kan mooier bertje
                    lookup = lookup.Replace("ORDER BY", String.Format(" WHERE {0}id = @@id@@ ORDER BY", parent))
                End If
                oRow = Me.objDS.Tables("SYS_RELATION").NewRow()
                oRow("parentname") = parent
                oRow("childname") = Child
                oRow("relationname") = parent + "->" + Child
                oRow("parentlookup") = lookup
                Me.objDS.Tables("SYS_RELATION").Rows.Add(oRow)
                Return True
            Catch ex As Exception
                DLA2EAHelper.Error2Log(ex)
                Return False
        ...
        Public Shared Function DataTable2Insert(Table As DataTable) As String
            ...
            For Each Column As DataColumn In Table.Columns
                'beetje lelijk met die validation maar werkt wel waarschijnlijk
                If Column.Expression.Length = 0 And Column.AutoIncrement = False And Not Column.ColumnName.ToUpper() = "VALIDATION" Then
        ...
        Public Shared Function DataTable2Update(Table As DataTable) As String
            ...
            For Each Column As DataColumn In Table.Columns
                If Column.Expression.Length = 0 And Column.AutoIncrement = False And Not Column.ColumnName.ToUpper() = "VALIDATION" Then
                    ...
                    strColumns += Column.ColumnName.ToLower() + " = " + Column2ParaMeter(Column)
                End If
            Next
            Template = Template.Replace("#fieldlist#", strColumns)
            Template = Template.Replace("#where#", Table2Where(Table))
        ...
        Public Shared Function DataTable2Detail(Table As DataTable) As String
            Dim Template As String = "SELECT #fieldlist# FROM #tablename# #where# "
            Template = Template.Replace("#tablename#", Table.TableName)
            Dim strColumns As String = ""
            For Each Column As DataColumn In Table.Columns
                If Column.Expression.Length = 0 And Not Column.ColumnName.ToUpper() = "VALIDATION" Then
                    strColumns = strColumns + IIf(strColumns.Length = 0, "", ", ")
                    strColumns = strColumns + Column.ColumnName.ToLower()
                End If
            Next
        ...
        Public Shared Function DataTable2List(Table As DataTable) As String
            Dim Template As String = "SELECT * FROM #tablename# "
            Template = Template.Replace("#tablename#", Table.TableName)
            Return Template
        End Function
        ''' <summary>
        ...
        End Function
        ''' <summary>
        ''' Create a system table for the relations for processing in the simulator and the form factory 
        ''' </summary>
        ''' <returns></returns>
        Private Function CreateRelationTable() As Boolean
            Dim DT As DataTable
            Try
                DT = Me.AddTable("SYS_RELATION")
                Me.AddColumn(DT, "relationname", "Relation name", "String")
                Me.AddColumn(DT, "parentname", "Parent name", "String")
                Me.AddColumn(DT, "childname", "Child name", "String")
                Me.AddColumn(DT, "parentlookup", "Parent lookup", "String")
                Me.AddColumn(DT, "childlookup", "Child lookup", "String")
                Return True
            Catch ex As Exception
                DLA2EAHelper.Error2Log(ex)
                Return False
        ...
        Public Function AddColumn(DT As DataTable, ColumnName As String, ColumnAlias As String, ColumnType As String) As DataColumn
            ...
            Try
                ...
                Select Case ColumnType
                    ...
                    Case "Boolean"
                        ColumnType = "Boolean"
                        objColumn.DataType = System.Type.GetType("System.Boolean")
                    Case Else
        ...
        End Function
        ''' <summary>
        ''' Transform a Lookup definitie to a SQL select statement
        ''' </summary>
        ''' <param name="Table"></param>
        ''' <param name="Lookup"></param>
        ''' <returns></returns>
        Public Function LookupToCommand(Table As String, Lookup As String) As Boolean
            Dim strTemplate As String = String.Format("SELECT {0}ID as ID, {1} as Description FROM {0} ORDER BY 2", Table, IIf(Lookup.Length > 0, Lookup, " * "))
            Me.AddCommand(Table, "LOOKUP", strTemplate)
            Return True
        End Function
        ...
        Public Function AddCalculatedColumn(DT As DataTable, ColumnName As String, ColumnAlias As String, ColumnType As String, Expression As String) As DataColumn
            ...
            Try
                ...
                End If
            Catch ex As Exception
                DLA2EAHelper.DebugAssertion(DT.TableName + ColumnName + Expression + ex.Message)
                DLA2EAHelper.Error2Log(ex)
            End Try
            Return Nothing
        End Function
        ...
        Public Function AddConditions(Element As EA.Element, DT As DataTable) As Boolean
            ...
            For Each Constraint As EA.Constraint In Element.Constraints
                strExpression += IIf(strExpression.Length > 0, " + ", "") +
                    "Iif(" + System.Net.WebUtility.HtmlDecode(Constraint.Notes) + ", 'Ok','" + Constraint.Name + "' )"
            Next
                    ...
                    For Each oCon In Element.Connectors
                        ...
                        If oCon.SupplierEnd.Cardinality.Contains("1") And oCon.ClientEnd.Cardinality.Contains("*") And oCon.SupplierID = Element.ElementID And oCon.Type = "Association" Then
                            ...
                            If oClient.PackageID = Package_id Then
                                ...
                                Me.AddRelation(Element.Name, oClient.Name, "")
                            End If
                        End If
                        If oCon.ClientEnd.Cardinality.Contains("1") _
                            ...
                            If oSupplier.PackageID = Package_id Then
                                ...
                                Me.AddRelation(Element.Name, oSupplier.Name, "")
                            End If
                        End If
                        ' Veel op veel
                        If oCon.SupplierEnd.Cardinality.Contains("*") And oCon.ClientEnd.Cardinality.Contains("*") And oCon.SupplierID = Element.ElementID And oCon.Type = "Association" Then
                            ...
                            If oClient.PackageID = Package_id Then
                                ...
                                'dummy tabell voor de link aanmaken
                                Dim LinkDT As DataTable
                                LinkDT = Me.AddTable(Element.Name + oClient.Name)
                                Me.AddControl(Element.Name + oClient.Name, Element.Name + oClient.Name + "id", "HiddenSLE", "", True, Element.Name)
                                ...
                                Me.AddCalculatedColumn(LinkDT, strConnectorName, strConnectorName, "String", "Parent([" + strConnectorName + "]).Lookup ")
                                'dit moet je nog verder uitzoeken
                                Me.AddRelation(Element.Name, Element.Name + oClient.Name, "")
                                Me.AddRelation(oClient.Name, Element.Name + oClient.Name, "")
                        ...
                        If oCon.ClientEnd.Cardinality.Contains("1") _
                            ...
                            If oSupplier.PackageID = Package_id Then
                                ...
                                Me.ContainerDataSet.Relations.Add(strConnectorName,
                                                                   ...
                                                                   Me.ContainerDataSet.Tables(oSupplier.Name).Columns(strPrimaryKey))
                                'Me.AddCalculatedColumn(Me.objDS.Tables(oSupplier.Name), strConnectorName, strConnectorName, "String", "Parent([" + strConnectorName + "]).Lookup ")
                                Me.AddRelation(Element.Name, Element.Name, "")
                            End If
                        End If
        ...
        End Function
        ''' <summary>
        ''' Transform the attribute type to a control for code generation
        ''' </summary>
        ''' <param name="Attribute">EA Attrbitute</param>
        ''' <returns>Controltype code</returns>
        ...
        Public Function Attributes2Dataset(Element As EA.Element, DT As DataTable, Optional prefix As String = "") As Boolean
            Try
                Dim attribute As EA.Attribute
                For Each attribute In Element.Attributes
                    Dim strControlType As String = AttributeType2Control(attribute)
