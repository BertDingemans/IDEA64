...
Namespace DLAFormfactory
    ''' <summary>
    ''' Class for encapsulating navigation through a recordset
    ''' can be used for example to fill list boxes etc.
    ''' </summary>
    ''' <remarks></remarks>
    Public Class DLASupplier
        ...
        ''' <summary>
        ''' Put the sql statement in the form of a dataadapter or datareader
        ''' with a nice error handling
        ''' </summary>
        ...
        Public Function SetStatement(ByVal strS As String, ByVal objCon As DbConnection) As String
            ...
            Try
                ...
                End If
                DLA2EAHelper.SQL2Log(strSql, "NO")

            Catch e As DbException
                DLA2EAHelper.Error2Log(e)
                strError = e.Message
                DLA2EAHelper.SQL2Log(strSql, e.Message)
            End Try
            Return strError
        End Function
    End Class
