...
Public Class FrmIdeaDiagram
        ...
        Else
            ...
            Try
                ...
                For Each item In Me.ListBoxElements.CheckedItems
                    ...
                    If CheckBoxFullModelCopy.Checked = True Then
                        objNew.Notes = objElement.Notes
                        objNew.Alias = objElement.Alias
                        objNew.Phase = objElement.Phase
                        objNew.Version = objElement.Version
                        objNew.Update()
                    End If
                    ...
                    objDT = DLA2EAHelper.SQL2DataTable(strSql, Me.Repository)
                    Dim objDR As DataRow
                    If Not objNew.Stereotype.ToUpper().Contains("ARCHIMATE") Then
                        For Each objDR In objDT.Rows
                        ...
                        If objCon.SupplierID = objElement.ElementID Then
                            ...
                            For Each arrFind In ElementsCombi
                                If arrFind(0).ElementID = intConnectorEnd Then
                                    ...
                                    objNewEnd = arrFind(1)
                                    objConNew = objGenerator.CopyAssociation(objCon, objNew, objNewEnd, CheckBoxFullModelCopy.Checked)
                ...
                Next
                Me.Repository.EnableUIUpdates = True
                Me.Repository.RefreshModelView(Me.ComboBoxTargetPackage.SelectedValue)
                Me.Close()
            Catch ex As Exception
                DLA2EAHelper.Error2Log(ex)
            End Try
    ...
    Private Sub ButtonCollectElements_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ButtonCollectElements.Click
        ...
        Try
            If DLA2EAHelper.IsUserGroupMember(Repository, "Administrators") Then
                ...
                Repository.EnableUIUpdates = False
                Dim blnSuppress As Boolean = My.Settings.SuppresWarningDialog
                ...
                For Each theDO In Diagram.DiagramObjects
                    ...
                    If Me.CheckBoxOriginalPackage.Checked = True Then
                ...
                Repository.ReloadDiagram(Diagram.DiagramID)
                Repository.EnableUIUpdates = True
                My.Settings.SuppresWarningDialog = blnSuppress
                My.Settings.Save()
            End If
        ...
        Try
            ...
            For Each objDL In Me.Diagram.DiagramLinks
                If Me.CheckBoxMakeHidden.Checked Then
                    ...
                    End If
                Else
                    If objDL.IsHidden Or objDL.Style = "T" Then
                        objDL.IsHidden = False
                        objDL.LineWidth = 5
                        objDL.LineColor = 100
                    End If
    ...
    Private Sub ButtonLoad_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ButtonLoad.Click
        Try
            ...
            Else
                MsgBox("Please select one row first (not a cell only)", MsgBoxStyle.OkOnly)
            End If
        Catch ex As Exception
    ...
    Private Function SQL2File(Statement As String, Template As String, filename As String) As String
        ...
        Try
            ...
            Else
                Dim strFile As String = oID.GetSettingValue("ReleaseDirectory") + "\" + filename + "-" + Me.Diagram.Name + ".sql"

                If Not System.IO.Directory.Exists(oID.GetSettingValue("ReleaseDirectory") + "\") Then
                    If MsgBox("Directory " + oID.GetSettingValue("ReleaseDirectory") + " does not exists, create it?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                        System.IO.Directory.CreateDirectory(oID.GetSettingValue("ReleaseDirectory") + "\")
                    Else
                        blnDoorgaan = False
                    End If
                End If
                If blnDoorgaan Then
                    DLA2EAHelper.String2File(strSQL, strFile, System.IO.FileMode.Create)
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles ButtonCardinality.Click
        Try
            Me.Repository.EnableUIUpdates = False
            Dim objDL As EA.DiagramLink
            For Each objDL In Me.Diagram.DiagramLinks
                Dim oCon As EA.Connector
                oCon = Me.Repository.GetConnectorByID(objDL.ConnectorID)
                If oCon.SupplierEnd.Cardinality.Length = 0 And oCon.ClientEnd.Cardinality.Length = 0 Then

                    If RadioButtonManyToMany.Checked Then
                        oCon.SupplierEnd.Cardinality = "0..*"
                        oCon.ClientEnd.Cardinality = "0..*"
                    ElseIf RadioButtonOneOne.Checked Then
                        oCon.SupplierEnd.Cardinality = "1"
